...
Early versions of the project relied on a fully synthetic dataset created
by :mod:`src.generate_dataset`.  The generator was tuned to produce
realistic‑looking trajectories and allowed the team to iterate quickly
without needing to obtain an institutional data use agreement.

The synthetic data exhibited the following patterns:
...
### 2.4 Real‑World Dataset: Open University Learning Analytics

To bring the prototype closer to practice we later adapted the pipeline to
work with the public Open University Learning Analytics (OULAD) dataset.
OULAD is composed of several tables (``studentInfo.csv``, ``vle.csv``,
``studentAssessment.csv`` etc.) covering tens of thousands of distance
learners.  Our data loader merges the relevant tables at the student level
and computes a small set of features such as:

* total / average VLE clicks
* number of sessions
* average assessment score

A binary target ``burnout_status`` is derived as a simple proxy: students
who eventually **withdraw** or **fail** the module are labelled as at-risk.
This is of course a rough approximation – one could instead use survey
responses from the DASS‑21 or CABB instruments if they were available – but
it allows the machine‑learning pipeline to be exercised on real behaviour
data.  Using OULAD typically produces AUC figures in the 0.65–0.80 range, a
much more realistic benchmark than the perfect scores seen on the synthetic
data.
