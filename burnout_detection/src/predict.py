...
class BurnoutPredictor:
    ...
    def __init__(self, model_path='../models/best_burnout_model.pkl',
        ...
        # load all encoders found in the models folder (prefix 'label_encoder_')
        self.encoders = {}
        model_dir = os.path.dirname(model_path)
        try:
            for fname in os.listdir(model_dir):
                if fname.startswith("label_encoder_") and fname.endswith(".pkl"):
                    col = fname[len("label_encoder_"):-4]
                    self.encoders[col] = joblib.load(os.path.join(model_dir, fname))
            if not self.encoders:
                print("No categorical encoders found (OK if data was all numeric)")
            else:
                print(f"Loaded encoders for columns: {list(self.encoders.keys())}")
        except Exception as e:
            print(f"Unable to scan for encoders: {e}")
            self.encoders = {}

        # Load feature names (strict ordering is important)
    ...
    def predict_single_student(self, student_data):
        ...
        X = student_data[self.feature_names].copy()

        # apply any encoders that were saved along with the model
        for col, enc in self.encoders.items():
            if col in X.columns:
                try:
                    X[col] = enc.transform(X[col].astype(str))
                except Exception:
                    # if transform fails we fall back to original values
                    pass
